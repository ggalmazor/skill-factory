...
3. **Apply a finished merger.** On a merger's completion: success → stamp the card's Timeline `merged:` with the current local time, move its task file `doing/`→`done/`, mark `done`; conflict **or verify-failed** → leave the file in `doing/`, mark `blocked`, surface to the user with the reason (conflicting paths, or the failing verify command). A verify-failed task's merge was rolled back, so the base is clean and the task branch still holds the work for a fix-and-retry. Clear the merger-active flag either way.
...
- **Merger subagent:** spawn with `model: "sonnet"` (mechanical integration), passing the card's **Verify** command if it has one. Merges one task's branch into that task's **Base** branch, runs the **Verify** gate before deleting anything (rolling the merge back if it fails), then deletes the worktree, updates `CHANGELOG` from the handoff, and reports success, conflict, or verify-failed. One at a time. Full prompt: [references/merger-subagent.md](references/merger-subagent.md).
